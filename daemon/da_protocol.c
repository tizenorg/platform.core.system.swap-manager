...
static int process_msg_get_probe_map(struct msg_buf_t *msg_control)
{
	enum ErrorCode err_code = ERR_NO;
	LOGW("packed_probe_map = %p; size = %u\n", &packed_probe_map, sizeof(struct packed_probe_map_t));
	sendACKToHost(NMSG_GET_PROBE_MAP, err_code, (void *)&packed_probe_map,
...
int host_message_handler(struct msg_t *msg)
{
	...
	switch (msg->id) {
	case NMSG_VERSION:
		sendACKToHost(msg->id, ERR_NO, PROTOCOL_VERSION,
			      sizeof(PROTOCOL_VERSION));
		break;
	case NMSG_GET_PROBE_MAP:
		return process_msg_get_probe_map(&msg_control);
	case NMSG_KEEP_ALIVE:
