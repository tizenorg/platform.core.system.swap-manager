...
static char *gen_target_info_reply(struct target_info_t *target_info, uint32_t *size)
{
	char *msg;
	char *p = NULL;

	msg = malloc(sizeof(*target_info) -
...
int process_msg_get_target_info(struct msg_buf_t *msg)
{
...
send_ack:
	if (sendACKToHost(msg->id, error_code, payload, payload_len) != 0) {
...
//		error_code = ERR_NO;
	...
	case NMSG_SWAP_INST_REMOVE:
		...
		goto send_ack;
	case NMSG_GET_TARGET_INFO:
		return process_msg_get_target_info(&msg_control);
