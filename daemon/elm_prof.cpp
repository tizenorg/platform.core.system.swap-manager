...
{
	return offsetof(struct appcore_ops, create);
}

static int get_reset_cb_offset(void)
{
	return offsetof(struct appcore_ops, reset);
}
...
struct app_info {
	app_info(const char *app_path, uint64_t main_addr,
		 const char *lib_path, uint64_t appcore_addr)
	{
		this->app_path = app_path;
		this->main_addr = main_addr;
		this->lib_path = lib_path;
		this->appcore_addr = appcore_addr;
	}
...
static int elm_prof_add_app(const struct app_info &info)
{
	...
	cmd = "a 0x" + int2hex(info.main_addr) + ":" + info.app_path + " " +
		"0x" + int2hex(info.appcore_addr) + ":" + info.lib_path;
...
{
	return elm_prof_rm_app(info.app_path);
}
...
static int sent_comm_info()
{
	...
	cmd = "s offset_reset " + int2str(get_reset_cb_offset());
	ret = write_to_file(path_cmd, cmd);
	if (ret < 0)
		return ret;
...
static int set_enable()
{
	...
	ret = write_to_file(path_enabled, "1");
...
static int set_disable()
{
	...
	ret = write_to_file(path_enabled, "0");
...
static int do_elm_prof_add(const app_info &info, app_info_list &app_list)
{
	...
	if (app_list.end() != app_info_list_find(app_list, info.app_path))
		return -EINVAL;
...
extern "C" int elm_prof_add(const char *app_path, unsigned long main_addr,
			    const char *lib_path, unsigned long appcore_addr)
{
	app_info info(app_path, main_addr, lib_path, appcore_addr);
