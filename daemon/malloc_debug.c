...
#define logi LOGI
#define loge LOGE
//#define logi(...) do{LOGI(__VA_ARGS__); sleep(2);}while(0)
//#define loge(...) do{LOGE(__VA_ARGS__); sleep(2);}while(0)
#else
#define logi(...)
#define loge(...)
#endif
...
void add_malloc(struct mlist_t **list, void *addr, int line, const char *file_name,
		const char *func_name, size_t size)
{
	...
	////LOGI("MALLOC: %04d) 0x%lX <%s>\n", el->line, el->addr, el->info);
	list_append(list, el);

}

void *malloc_call_d(int line, const char *file_name, const char * func_name, size_t size)
{
	logi("malloc>\n");
...
void *calloc_call_d(int line, const char *file_name, const char * func_name, size_t n, size_t size)
{
	...
	add_malloc(&(file_el->addr), addr, line, file_name, func_name, size * n);

	return addr;
}

void free_call_d(int line, const char *file_name, const char * function, void *addr)
{
