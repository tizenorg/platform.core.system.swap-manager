...
static void *replay_thread(void *arg)
{
	...
	LOGI_th_rep("\ttime=0x%08X %08X (%u.%u), count=%u\n",
		    (unsigned int)start_tv.tv_sec,  /* start time sec (hex) */
		    (unsigned int)start_tv.tv_usec, /* start time usec (hex) */
		    (unsigned int)start_tv.tv_sec,  /* start time sec (dec) */
		    (unsigned int)start_tv.tv_usec, /* start time usec (dec) */
		    event_num			    /* events count */
		);
