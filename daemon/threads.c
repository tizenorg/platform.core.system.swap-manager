...
static void* recvThread(void* data)
{
	...
	for (;;) {
		...
		else if(log.type == MSG_TERMINATE)
		{
			...
			continue;
		} else if (log.type == MSG_GET_UI_HIERARCHY_DATA) {
			char *file_name = log.data;
			FILE * fp;
			...
			if (len == sizeof(uint32_t)) {
				...
				LOGE("MSG_GET_UI_HIERARCHY_DATA error <%d>\n", err_code);
				// TODO: system error
			...
			if (access(file_name, F_OK) != -1) {
				LOGI("MSG_GET_UI_HIERARCHY_DATA> File: <%s>\n",
				     file_name);
				...
				LOGE("MSG_GET_UI_HIERARCHY> File not found <%s>\n",
				     file_name);
			}
			...
			LOGI("MSG_GET_UI_SCREENSHOT> log length = %d\n", log.length);
