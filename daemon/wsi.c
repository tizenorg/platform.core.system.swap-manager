...
int wsi_set_profile(const struct app_info_t *app_info)
{
	...
	snprintf(info_tmp, sizeof(info_tmp), "%s %s", app_info->exe_path,
		 app_info->app_id);
	ret = set_profile_info(APP_INFO_FILE, info_tmp);
	if (ret)
		goto fail;

	sprintf(info_tmp, "%lx", INSPECTOR_ADDR);
	ret = set_profile_info(INSPSERVER_START_FILE, info_tmp);
	if (ret)
		goto fail;

	sprintf(info_tmp, "%lx", WILLEXECUTE_ADDR);
	ret = set_profile_info(WILL_EXECUTE_FILE, info_tmp);
	if (ret)
		goto fail;

	sprintf(info_tmp, "%lx", DIDEXECUTE_ADDR);
