...
int init_daemon_dfs (struct daemon_fds *d_fds) {
	...
	/* Open swap buffer  */
	if ((d_fds->buf_fd = open(BUF_FILENAME, O_RDONLY | O_CLOEXEC)) == -1) {
	...
	if ((d_fds->conf_fd = open(CONF_FILENAME, O_RDONLY)) == -1) {
                log_errno ("Open swap config failed", errno);
                return -1;
        }

	if ((d_fds->trace_fd = open(TRACE_FILENAME, O_WRONLY | O_CREAT | O_TRUNC, S_IWUSR|S_IRUSR|S_IRGRP|S_IROTH)) == -1) {
...
int main () {
	...
	if (FD_ISSET(da_fds.self_pipe_fd[0], &read_fds)) {
		...
		if (ioctl_send_msg (da_fds.buf_fd, (const char *)stop_msg) == -1)
			exit (EXIT_FAILURE);
		...
		if (fcntl(da_fds.buf_fd, F_SETFL, saved_flags | O_NONBLOCK) == -1)
