...
check_null_or_exit()
{
	...
	if [ -z "$val" ]; then
		echo "ERROR: value '$name' is null" >&2
		exit 1
	fi
}
...
check_null_or_exit path_app_core_efl $path_app_core_efl

addr_appcore_efl_main=$(readelf -s ${path_app_core_efl} | grep appcore_efl_main | awk '{print $2}' | head -1)
check_null_or_exit addr_appcore_efl_main $addr_appcore_efl_main
...
check_null_or_exit path_launchpad $path_launchpad
...
check_null_or_exit addr_dlopen_plt $addr_dlopen_plt
check_null_or_exit addr_dlsym_plt $addr_dlsym_plt
