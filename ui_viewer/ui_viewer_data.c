...
static Eina_Bool _get_redraw(const char *type_name)
{
...
static void _render_obj(Evas *evas, Evas_Object *obj, const char *type_name,
			struct timeval *tv)
{
	Eina_Bool redraw;

	redraw = _get_redraw(type_name);
...
static char *_pack_ui_obj_info(char *to, Evas_Object *obj, Eina_Bool rendering)
{
	...
	if (rendering)
		_render_obj(evas, obj, info.name, &(info.rendering_tv));
	else
