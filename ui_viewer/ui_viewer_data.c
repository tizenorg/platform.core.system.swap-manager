...
static char *pack_ui_obj_prop(char *to, Evas_Object *obj);
...
static Eina_Bool _get_redraw(Evas_Object *obj)
{
	...
	if (strcmp(evas_object_type_get(obj), "edje"))
		redraw = EINA_TRUE;

	return redraw;
}

static void _render_obj(Evas *evas EINA_UNUSED, Evas_Object *obj, struct timeval *tv)
{
	Eina_Bool redraw;
...
static char *_pack_ui_obj_info(char *to, Evas_Object *obj, Eina_Bool rendering)
{
	ui_obj_info_t info;
	Evas *evas;
	...
	if (rendering)
		_render_obj(evas, obj, &(info.rendering_tv));
	else
		timerclear(&(info.rendering_tv));
	...
	if (_get_obj_exists(obj)) {
		...
		to = pack_ui_obj_prop(to, info.id);
	}

	return to;
}

char *pack_ui_obj_info_list(char *to, Eina_Bool rendering)
{
	Eina_List *obj_list, *obj_list_tmp;
	Evas_Object *obj;
	unsigned int info_cnt;
	char *cnt_ptr, *prev_ptr;
	unsigned int cur = 0;

	obj_list = ui_viewer_get_all_objs();
	info_cnt = eina_list_count(obj_list);

	EINA_LIST_FOREACH(obj_list, obj_list_tmp, obj)
	{
		evas_object_ref(obj);
	}
	...
	EINA_LIST_FOREACH(obj_list, obj_list_tmp, obj)
	{
		...
		// check if hierarchy request is cancelled
		if (get_hierarchy_status() == HIERARCHY_RUNNING) {
			...
			ui_viewer_log("break packing, objects count = %d\n", cur);
			pthread_mutex_unlock(&request_lock);
			break;
		}
		pthread_mutex_unlock(&request_lock);
	}
	// replace objects count if packing
	if (cur != info_cnt)
		pack_int32(cnt_ptr, cur);
...
bool is_getting_ui_hierarchy(void)
{
	bool is_getting;

	pthread_mutex_lock(&request_lock);
	is_getting = hierarchy_is_active;
	pthread_mutex_unlock(&request_lock);

	return is_getting;
}

void activate_getting_ui_hierarchy(void) {
	pthread_mutex_lock(&request_lock);
...
void cancel_getting_ui_hierarchy(void) {
	pthread_mutex_lock(&request_lock);

	hierarchy_is_active = false;
...
char *pack_ui_obj_screenshot(char *to, Evas_Object *obj)
{
	...
	if (!exists) {
		...
		win_id = _get_win_id(evas_object_evas_get(obj));
