...
char *pack_ui_obj_prop(char *to, Evas_Object *obj)
{
	...
	prop.id = obj;
	if (captureScreen(prop.screenshot) != 0)
		prop.screenshot[0] = '\0';
	evas_object_geometry_get(obj, &prop.geometry[0], &prop.geometry[1],
				 &prop.geometry[2], &prop.geometry[3]);
	prop.focus = evas_object_focus_get(obj);
	prop.name = evas_object_name_get(obj);
	prop.visible = evas_object_visible_get(obj);
	evas_object_color_get(obj, &prop.color[0], &prop.color[1],
			      &prop.color[2], &prop.color[3]);
	prop.anti_alias = evas_object_anti_alias_get(obj);
	prop.scale = evas_object_scale_get(obj);
	evas_object_size_hint_min_get(obj, &prop.size_min[0], &prop.size_min[1]);
	evas_object_size_hint_max_get(obj, &prop.size_max[0], &prop.size_max[1]);
	evas_object_size_hint_request_get(obj, &prop.size_request[0], &prop.size_request[1]);
	evas_object_size_hint_align_get(obj, &prop.size_align[0], &prop.size_align[1]);
	evas_object_size_hint_weight_get(obj, &prop.size_weight[0], &prop.size_weight[1]);
	evas_object_size_hint_padding_get(obj, &prop.size_padding[0], &prop.size_padding[1],
					  &prop.size_padding[2], &prop.size_padding[3]);
	prop.render_op = evas_object_render_op_get(obj);

	to = pack_ui_obj_evas_prop(to, &prop);
