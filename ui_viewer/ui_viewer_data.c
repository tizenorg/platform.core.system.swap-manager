...
// we set window as parent for objects without it
static Eina_List *_get_extra_win_children(Evas_Object *win_obj)
{
	static Eina_List *children = NULL;
	Evas_Object *obj_i;
	Evas *evas;
	Eina_List *objs;

	if (strcmp(evas_object_type_get(win_obj), "elm_win"))
		return NULL;

	evas = evas_object_evas_get(win_obj);

	objs = evas_objects_in_rectangle_get(evas, SHRT_MIN, SHRT_MIN, USHRT_MAX, USHRT_MAX,
					     EINA_TRUE, EINA_TRUE);

	EINA_LIST_FREE(objs, obj_i)
	{
		if (evas_object_smart_parent_get(obj_i) == 0 &&
		    strcmp(evas_object_type_get(obj_i), "elm_win"))
			children = eina_list_append(children, obj_i);
	}
	eina_list_free(objs);
...
char *pack_ui_obj_prop(char *to, Evas_Object *obj)
{
	...
	ui_viewer_log("get_object_properties for 0x%lx, exists : %d, type : %s\n",
		(unsigned long)obj, exists, evas_object_type_get(obj));
	...
	if (ui_viewer_capture_screen(obj_prop.screenshot, obj) != 0)
...
static void _render_obj(Evas *evas, Evas_Object *obj, struct timeval *tv)
{
	...
	Eina_Bool was_visible;

	was_visible = evas_object_visible_get(obj);
			       ...
			       Evas_Object *obj, Evas_Object *parent)
{
	...
	if (!strcmp(evas_object_type_get(obj), "elm_win")) {
		children = _get_extra_win_children(obj);
		EINA_LIST_FREE(children, child) {
			render_list = _get_rendering_time(render_list, evas, child, obj);
		}
		eina_list_free(children);
