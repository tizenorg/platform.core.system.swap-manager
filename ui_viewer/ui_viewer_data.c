...
static Eina_Bool _get_redraw(Evas_Object *obj)
{
	...
	if (elm_object_widget_check(obj) &&
	    strcmp(evas_object_type_get(obj), "elm_win") &&
	    strcmp(evas_object_type_get(obj), "elm_access") &&
	    strcmp(evas_object_type_get(obj), "elm_gesture_layer"))
		redraw = EINA_TRUE;
...
static Eina_List *_get_obj_info(Eina_List *ui_obj_info_list, Evas_Object *obj)
{
	...
	fprintf(stderr, "object_id : 0x%lx, object_category : %d, object_type_code: %d, "
			"object_type_name : %s, object_possible_redraw : %d, "
			"parent_object_id : 0x%lx\n",
			(unsigned long) obj,
...
ui_obj_prop_t *ui_viewer_get_object_properties(Evas_Object *obj)
{
	...
	fprintf(stderr, "object_id : 0x%lx, screenshot : %s, geometry: [%d,%d,%d,%d], "
			       ...
			       Evas_Object *obj, Evas_Object *parent)
{
	...
	if (!render_info) {
		fprintf(stderr, "Cannot alloc ui_obj_render_info_t: %d bytes\n",
	...
	timersub(&finish_tv, &start_tv, &(render_info->tv));

	fprintf(stderr, "obj: 0x%lx, parent : 0x%lx, rendering time: %lf\n",
		(unsigned long)obj, (unsigned long)parent,
		render_info->tv.tv_sec * 1000000.0 + render_info->tv.tv_usec);
