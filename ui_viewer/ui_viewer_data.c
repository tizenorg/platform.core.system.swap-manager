...
char *pack_ui_obj_prop(char *to, Evas_Object *obj)
{
	...
	if (category == UI_ELM) {
		...
		edje_prop.scale = (float)edje_object_scale_get(obj);
		edje_prop.base_scale = (float)edje_object_base_scale_get(obj);
		edje_object_size_min_get(obj, &coord[0], &coord[1]);
		edje_prop.size_min[0] = (uint32_t)coord[0];
		edje_prop.size_min[1] = (uint32_t)coord[1];
		edje_object_size_max_get(obj, &coord[0], &coord[1]);
		edje_prop.size_max[0] = (uint32_t)coord[0];
		edje_prop.size_max[1] = (uint32_t)coord[1];
