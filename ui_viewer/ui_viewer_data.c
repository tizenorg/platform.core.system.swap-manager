...
Eina_List *ui_viewer_get_rendering_time(Evas_Object *obj)
{
	...
	Eina_Bool exists, redraw;
	Evas *evas;

	exists = _check_obj_exists(obj);
	redraw = _get_redraw(obj);

	fprintf(stderr, "get rendering time for 0x%lx, exists : %d, redraw : %d\n",
...
{
	fprintf(stderr, "pack obj: 0x%lx, parent : 0x%lx, rendering time: %lu sec, %lu usec\n",
