		...
		else if(FD_ISSET(gTraceInfo.socket.daemonSock, &workfds))
		{
			...
			if(recvlen > 0)	// recv succeed
			{
				...
				if (log.type == APP_MSG_CONFIG) {
					...
					if (err_code != ERR_NO) {
						PRINTERR("APP_MSG_GET_UI_PROPERTIES error <%d>", err_code);
						continue;
					}

					if(log.length > 0) {
						Eina_Bool rendering;
						pthread_t g_hierarchy_thread_id;

						rendering = (Eina_Bool)*((uint8_t *)data_buf);
						PRINTMSG("APP_MSG_GET_UI_HIERARCHY, rendering option <%d>", rendering);
						if (pthread_create(&g_hierarchy_thread_id, NULL, print_log_ui_viewer_info_list,
								   (void*)&rendering))
							PRINTMSG("failed to crate hierarchy thread");
					} else {
