...
__attribute__((constructor)) void init_ui_viewer(void)
{
	...
	// init sockMutex
	pthread_mutex_init(&ui_gTraceInfo.socket.sockMutex, NULL);
...
__attribute__((destructor)) void finite_ui_viewer (void)
{
	...
	if (ui_gTraceInfo.socket.daemonSock != -1)
		close(ui_gTraceInfo.socket.daemonSock);
}
...
{
	ui_gTraceInfo.optionflag = atoll(configstr);
}
...
static int createSocket(void)
{
	...
	ui_gTraceInfo.socket.daemonSock =
	...
	if (ui_gTraceInfo.socket.daemonSock != -1)	{
		...
		if (connect(ui_gTraceInfo.socket.daemonSock,
			    (struct sockaddr *)&clientAddr, clientLen) >= 0)
		{
			...
			ui_print_log_str(MSG_PID, buf);

			while ((recved & MSG_CONFIG_RECV) == 0) {
				PRINTMSG("wait incoming message %d\n",
					 ui_gTraceInfo.socket.daemonSock);
				recvlen = recv(ui_gTraceInfo.socket.daemonSock, &log,
				...
				if (recvlen > 0) {
					...
					recvlen = recv(ui_gTraceInfo.socket.daemonSock, data_buf,
					...
					if (log.type == MSG_OPTION) {
						PRINTMSG("MSG_OPTION 8.0");
						_configure(data_buf);
...
free_data_buf:
					...
					close(ui_gTraceInfo.socket.daemonSock);
					ui_gTraceInfo.socket.daemonSock = -1;
...
static void *recvThread(void __unused *data)
{
	...
	if(maxfd < ui_gTraceInfo.socket.daemonSock)
		maxfd = ui_gTraceInfo.socket.daemonSock;
	FD_SET(ui_gTraceInfo.socket.daemonSock, &readfds);
	...
	while(1)
	{
		...
		else if(FD_ISSET(ui_gTraceInfo.socket.daemonSock, &workfds))
		{
			recvlen = recv(ui_gTraceInfo.socket.daemonSock, &log,
			...
			if(recvlen > 0)	// recv succeed
			{
				...
				if(log.type == MSG_STOP) {
				...
				} else if(log.type == MSG_GET_UI_HIERARCHY) {
					enum ErrorCode err_code = ERR_UNKNOWN;

					print_log_ui_viewer_hierarchy_status(&err_code);

					if (err_code != ERR_NO) {
						PRINTERR("MSG_GET_UI_HIERARCHY error <%d>", err_code);
						continue;
					}
					...
					if(log.length > 0) {
						...
						PRINTMSG("MSG_GET_UI_HIERARCHY, rendering option <%d>", rendering);
						raise_app_window();
						...
						PRINTERR("WRONG MSG_GET_UI_PROPERTIES");
					}

					continue;
				} else if(log.type == MSG_GET_UI_HIERARCHY_CANCEL) {
					PRINTMSG("MSG_GET_UI_HIERARCHY_CANCEL");
					...
					continue;
				} else if(log.type == MSG_GET_UI_SCREENSHOT) {
					if(log.length > 0) {
						Evas_Object *obj;

						obj = (Evas_Object*)(unsigned long)*((uint64_t *)data_buf);
						PRINTMSG("MSG_GET_UI_SCREENSHOT <0x%lx>", obj);
						raise_app_window();
						print_log_ui_obj_screenshot(obj);
					} else {
						PRINTERR("WRONG MSG_GET_UI_SCREENSHOT");
					}
