...
__attribute__((constructor)) void init_ui_viewer(void)
{
	...
	fprintf(stderr, "constructor started\n");
...
{
	fprintf(stderr, "destructor started\n");
			...
			if(recvlen > 0)	// recv succeed
			{
				...
				if (log.type == APP_MSG_CONFIG) {
					...
					if(log.length > 0) {
						...
						rendering_list = ui_viewer_get_rendering_time(obj);
