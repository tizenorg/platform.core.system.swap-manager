...
bool print_log_ui_viewer_info_list(int msgType, Eina_Bool rendering)
{
	log_t log;
	ssize_t res, len;
	...
	ui_viewer_log("getting hierarchy time : %d sec, %d usec, filename: %s filesize: %d\n",
		      tv.tv_sec, tv.tv_usec, log.data, tmp_ptr - tmp_buf);
...
bool print_log_ui_obj_screenshot(int msgType, Evas_Object *obj)
{
	log_t log;
	ssize_t res, len;
	char *log_ptr;
