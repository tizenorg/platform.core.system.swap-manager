...
extern __traceInfo ui_gTraceInfo;
...
bool ui_printLog(log_t *log, int msgType)
{
	ssize_t res, len;
	...
	pthread_mutex_lock(&(ui_gTraceInfo.socket.sockMutex));
	res = send(ui_gTraceInfo.socket.daemonSock, log, len, 0);
	pthread_mutex_unlock(&(ui_gTraceInfo.socket.sockMutex));

	return (res == len);
}

bool ui_print_log_str(int msgType, char *str)
{
	log_t log;
	ssize_t res, len;
...
bool ui_print_log_fmt(int msgType, const char *func_name, int line, ...)
{
	ssize_t res = 0, len = 0;
	char *fmt, *p;
	int n;
	...
	if (ui_gTraceInfo.socket.daemonSock != -1) {
		res = send(ui_gTraceInfo.socket.daemonSock, &log, len, MSG_DONTWAIT);
	} else {
		ui_viewer_log("%d %s\n", msgType, log.data);
		fflush(stderr);
	}
...
bool print_log_ui_viewer_hierarchy_error(void)
{
	...
	log.type = MSG_GET_UI_HIERARCHY_DATA;
	log_ptr = pack_int32(log.data, (uint32_t)err_code);
	log.length = log_ptr - log.data;
	len = sizeof(log.type) + sizeof(log.length) + log.length;
...
bool print_log_ui_obj_screenshot(Evas_Object *obj)
{
	...
	log.type = MSG_GET_UI_SCREENSHOT;
